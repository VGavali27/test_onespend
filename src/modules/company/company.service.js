...
export const create = async (data) => {
  ...
  if (!group) throw ApiError.notFound('Referenced group not found');

  if (data.code) {
    const existing = await companyRepository.findByCode(data.code);
    if (existing) throw ApiError.conflict('Company code already exists');
  }
...
export const deleteRecord = async (uuid) => {
  const deleted = await companyRepository.deleteRecord(uuid);
