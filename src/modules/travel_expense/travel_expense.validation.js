...
// Child item schemas for combined creation
const segmentSchema = Joi.object({
  ...
  preferred_vendor: Joi.string().allow(null, ''),
  preferred_number: Joi.string().allow(null, ''),
  seat_preference: Joi.string().allow(null, ''),
  meal_preference: Joi.string().allow(null, ''),
...
const accommodationSchema = Joi.object({
  ...
  property_name: Joi.string().allow(null, ''),
...
// Schema for combined create — expense + travel + all child items
export const createExpenseWithTravelSchema = Joi.object({
  // Expense fields
  category_uuid: Joi.string().uuid().required(),
  company_uuid: Joi.string().uuid().required(),
  requested_by_user_uuid: Joi.string().uuid().required(),
  title: Joi.string().max(255).required(),
  remarks: Joi.string().allow(null, ''),
  estimated_amount: Joi.string().required(),
  // Travel fields
  travel_type: Joi.string().max(30).required(),
  purpose: Joi.string().required(),
  travel_start_date: Joi.date().iso().required(),
  travel_end_date: Joi.date().iso().required(),
  total_travellers: Joi.number().integer().default(1),
  notes: Joi.string().allow(null, ''),
  // Child items (all optional arrays)
...
// Schema for standalone travel expense update
export const updateTravelExpenseSchema = Joi.object({
  travel_type: Joi.string().max(30),
  purpose: Joi.string(),
  travel_start_date: Joi.date().iso(),
  travel_end_date: Joi.date().iso(),
  total_travellers: Joi.number().integer(),
  notes: Joi.string().allow(null, ''),
}).min(1);
