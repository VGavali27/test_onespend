...
class TravelExpenseService {
  ...
  async getByExpenseUuid(expenseUuid) {
    const travelExpense = await travelExpenseRepository.findByExpenseUuid(expenseUuid);
    if (!travelExpense) throw ApiError.notFound('Travel expense not found');
  ...
  async createWithTravel(data) {
    ...
    return sequelize.transaction(async (t) => {
      ...
      // 3. Bulk create child items
      if (data.segments?.length) {
        await db.TravelExpenseSegment.bulkCreate(
          data.segments.map((s) => ({ ...s, travel_expense_id: travelExpense.id })),
          { transaction: t },
        );
      }
      if (data.accommodations?.length) {
        await db.TravelExpenseAccommodation.bulkCreate(
          data.accommodations.map((a) => ({ ...a, travel_expense_id: travelExpense.id })),
          { transaction: t },
        );
      }
      if (data.local_transports?.length) {
        await db.TravelExpenseLocalTransport.bulkCreate(
          data.local_transports.map((lt) => ({ ...lt, travel_expense_id: travelExpense.id })),
          { transaction: t },
        );
      }
      if (data.forex?.length) {
        await db.TravelExpenseForex.bulkCreate(
          data.forex.map((f) => ({ ...f, travel_expense_id: travelExpense.id })),
          { transaction: t },
        );
      }
      if (data.misc_expenses?.length) {
        await db.TravelExpenseMiscExpense.bulkCreate(
          data.misc_expenses.map((m) => ({ ...m, travel_expense_id: travelExpense.id })),
          { transaction: t },
        );
      }

      // Return expense with all nested data
      return expense;
    });
  }
