...
// Schema for creating a user employment — user_uuid, company_uuid required
export const createUserEmploymentSchema = Joi.object({
  user_uuid: Joi.string().uuid().required().messages({
  ...
  reporting_manager_employment_uuid: Joi.string().uuid().allow(null, '').messages({
    'string.guid': 'Reporting manager UUID must be a valid UUID',
  }),
  ...
  joining_date: Joi.date().iso().allow(null, '').messages({
    'date.format': 'Joining date must be a valid ISO date',
  }),
  relieving_date: Joi.date().iso().allow(null, '').messages({
    'date.format': 'Relieving date must be a valid ISO date',
  }),
});
...
export const updateUserEmploymentSchema = Joi.object({
  company_uuid: Joi.string().uuid().messages({
    'string.guid': 'Company UUID must be a valid UUID',
  }),
