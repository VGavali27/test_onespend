...
/**
 * Shared Department form (Add + Edit). React Hook Form + Zod.
 * Fields mirror backend src/modules/department/department.validation.js.
 */
export default function DepartmentForm({
  ...
  const handleFormSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      const baseMessage = err?.response?.data?.message || 'Failed to save department. Please try again.';
      if (mapped) {
  ...
  return (
    <>
      ...
      <form id="department-form" noValidate onSubmit={handleFormSubmit} className="space-y-6">
        <FormSection icon={Tags} title="Department" subtitle="Basic department details">
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            <FormField label="Department name" required error={errors.name?.message}>
              <input className={inputClassFor(!!errors.name)} {...register('name')} placeholder="e.g. Finance" />
            </FormField>
            <FormField label="Code" required error={errors.code?.message}>
              <input className={inputClassFor(!!errors.code)} {...register('code')} placeholder="e.g. FIN" />
            </FormField>
