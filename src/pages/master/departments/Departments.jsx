...
export default function Departments() {
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
          <Link
            to={`/master/departments/${row.original.uuid}`}
  ...
  const fetchDepartments = async ({ page, limit, sortBy, sortOrder, search }, { signal }) => {
    const { data } = await departmentApi.list({}, { signal });
    let rows = data?.data ?? [];
    if (search) {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await departmentApi.remove(deleteTarget.uuid);
