...
export default function Roles() {
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
          <Link to={`/access/roles/${row.original.uuid}`} title="View role" className="p-1.5 rounded-md text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors">
            <Eye className="h-4 w-4" />
          </Link>
          <Link to={`/access/roles/${row.original.uuid}/edit`} title="Edit role" className="p-1.5 rounded-md text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors">
  ...
  const fetchRoles = async ({ page, limit, sortBy, sortOrder, search }, { signal }) => {
    const { data } = await roleApi.list({}, { signal });
    let rows = data?.data ?? [];
    if (search) {
      const q = search.toLowerCase();
      rows = rows.filter((r) => [r.name, r.code, r.description].filter(Boolean).some((v) => String(v).toLowerCase().includes(q)));
    }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await roleApi.remove(deleteTarget.uuid);
  ...
  return (
    <>
      <DataTablePage
        title="Roles"
        subtitle="Manage roles and their access"
        icon={Shield}
        columns={columns}
        fetchFn={fetchRoles}
        countLabel="role"
        ...
        reloadKey={reloadKey}
        actions={
          <Link to="/access/roles/new" className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg text-[13px] font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-600/20 transition-colors">
