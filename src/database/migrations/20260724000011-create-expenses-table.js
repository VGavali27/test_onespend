export async function up(queryInterface, Sequelize) {
  await queryInterface.createTable('expenses', {
    ...
    paid_amount: { type: Sequelize.TEXT, allowNull: true, defaultValue: '0' },
    status: { type: Sequelize.STRING(30), allowNull: false, defaultValue: 'DRAFT' },
    payment_status: {
      type: Sequelize.ENUM('UNPAID', 'PARTIAL_PAID', 'PAID', 'ADVANCE_REFUND_DUE', 'ADDITIONAL_PAYMENT_DUE', 'SETTLED'),
      allowNull: false,
      defaultValue: 'UNPAID',
    },
...
export async function down(queryInterface, _Sequelize) {
  // Remove ALL FKs from any table referencing expenses before dropping expenses
  const [constraints] = await queryInterface.sequelize.query(`
    ...
    WHERE rc.REFERENCED_TABLE_NAME = 'expenses'
  `);
