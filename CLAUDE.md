...
### Today's Updates (2026-09-18) — FIXED-flow delegation (Model A)
Step owners in the FIXED (ladder) approval flow can now temporarily delegate their step to a junior — the junior's approve **returns the expense to the owner**. Ladder position (`flow_position`) never moves during the delegation window; only the owner's approve advances.

- **Schema** — `expenses.delegated_from_role_id` BIGINT UNSIGNED NULL (FK `fk_exp_delegated_from_role_id` → roles, index `idx_expenses_delegated_from_role`) in `expense.model.js` + migration `20260724000011-create-expenses-table.js`; live DB ALTERed. Association `delegatedFromRole` (belongsTo Role). Added to `listInclude` (`required: false`) in `expense.repository.js` — `detailInclude` spreads `listInclude`, so the detail endpoint surfaces it (the frontend's `normalizeExpense` reads `e.delegatedFromRole`/`e.delegated_from_role_id`).
- **Rules** — seeder `20260724000010-seed-role-handover-rules.js` gained module **`expense_delegation`** (uuid prefix `e1e2f3a4-b5c6-7890-cdef-12345678`), built by `buildDelegationRules`: 106→107 (ADMIN_MGR→ADMIN_JR), 104→105 (FINANCE_MGR→FINANCE_JR), 102→103 (PAYMENT_MGR→PAYMENT_JR), 101→104 (CFO→FINANCE_MGR). MODULES entries now carry a `build` fn. Live-inserved (4 rows).
- **Service** (`expense.service.js`):
  - `getValidDelegateRoles(uuid)` — valid targets `[{ roleId, roleUuid, roleName, roleCode }]` from `role_handover_rules` where `module='expense_delegation'`, `from_role_id = current_role_id`, ACTIVE; returns `[]` when the expense is already delegated or it's a HANDOVER/non-procurement expense.
  - `delegate(uuid, user, toRoleId, remarks)` — step owner only (role gate + `fromRole !== actorRole`), rejects if already `delegated_from_role_id != null` ("This expense is already delegated — return it before delegating again"), sets `current_role_id = toRoleId`, `delegated_from_role_id = fromRole`, logs `DELEGATE` via `logExpenseHandover`. `flow_position` untouched.
  - `approveFixedFlow` — added a return branch at the top of the transaction: when `expense.delegated_from_role_id != null` the approve restores the owner (`current_role_id = delegated_from_role_id`, flag → null, logs `DELEGATE_RETURN`), **without** advancing `flow_position` or running the advance gates. Gate changed from `expense.current_role_id !== handlerRole?.id` to `fromRole !== actorRole?.id` (unused `handlerRole` removed).
  - `reject()` and `submit()` now explicitly clear `delegated_from_role_id: null` alongside `current_role_id` (a rejection routes to ADMIN_MGR; a (re)submit restarts the ladder — no stale delegation survives either).
- **Controller + routes** — `getDelegateRoles` (`GET /expenses/:uuid/delegate-roles`) and `delegateExpense` (`POST /expenses/:uuid/delegate`, both `requirePermission('expenses:approve')`, body validated by the existing `actionSchema` with `to_role_id`).
- **Visibility fix** — `getByUuid` and `getProcurementChain` previously 404'd any viewer who wasn't a global role / the requester / employed in the expense's company. The **current handler** (`expense.current_role_id === role.id`) is now an allowed viewer, so a delegated junior (possibly employed in a different company) can actually open the detail they're asked to review.
- **Verified live** (E2E + smoke on EXP-20260918-0001 / `880829a5-f502-449b-98da-df1b965afa82`): delegate → JR views detail with `delegatedFromRole` → JR's `delegate-roles` is empty → JR approve returns to ADMIN_MGR (flag null, flow 2) → `DELEGATE` + `DELEGATE_RETURN` rows in `expense_handovers` (action_type is a plain string column — no ALTER needed). Ordinary advance gates (invoice + received) still fire on the owner's approve-after-return.

### Today's Updates (2026-09-17) — Nullable payment method + dedicated reimbursement remarks key
- **`reimbursement_expenses.payment_method` is now nullable.** Previously the column was `allowNull:false, defaultValue:'CASH'` (model + migration `20260804000001`), so every reimbursement expense — even one with no advance at all — stored `'CASH'`. Both the Sequelize model (`reimbursement_expense.model.js`) and the migration are now `allowNull:true, defaultValue:null`; the **live DB was already altered** (`ALTER TABLE reimbursement_expenses MODIFY payment_method VARCHAR(20) NULL DEFAULT NULL`; no backfill needed — 0 affected rows). The read-model/API never render it, so no other consumer changed. Joi now allows `null`/empty on **both** `reimbursement.validation.js` (create+update) and `expense.validation.js` (create+update) (the latter at `:198`, `:233`).
- **New `reimbursement_remarks` API key for the reimbursement remarks.** Before, the frontend's reimbursement `remarks` form field was never sent anywhere, and the backend copied the **expense-level** `remarks` into `reimbursement_expenses.remarks` on create — which also meant it leaked from the exp-level field. The create+update validation (`expense.validation.js:199`, `:234`) now accept `reimbursement_remarks: Joi.string().allow(null,'')`, and it is **destructured off the expense body** in both service paths (`expense.service.js:479` create, `:608` update) so it never lands on the expense row.
- **Null-safe create path** (`expense.service.js:452-459,559`): `reimbursementFields` now sets `payment_method = data.payment_method || null` and `remarks = data.reimbursement_remarks || null` (the old `'CASH'` fallback for payment_method is gone). Also fixed a TDZ bug: the create branch originally re-read `reimbursement_remarks` from `data` before the destructure — it now uses the destructured variable.
- **Null-safe update path** (`expense.service.js:714-722`): `reFields.payment_method = payment_method || null` (sends null when the client clears an advance's payment method) and `reFields.remarks = reimbursement_remarks` only **when the key is present** (`if (reimbursement_remarks !== undefined)`), so edits that don't touch remarks leave the stored value alone — and a client can explicitly clear it with `reimbursement_remarks: null`.
- **Frontend contract** — `POST/PUT /expenses` sends `advance_amount`, `advance_date`, `payment_method`, and `reimbursement_remarks` **only when `advance_amount > 0`**; when there is no advance those keys are omitted entirely and the backend leaves all four advance columns NULL (see frontend 2026-09-17 for the gated form UI).

### Today's Updates (2026-09-16) — Pending-only notification bell + handover context
- **`notification.service.js` — the bell shows ONLY what's currently pending at the user's role.** The history/"activity" feed is **removed**: `getActivityRows`, its helpers (`parseProcurementParent`, chain-key resolvers `getExpenseChainKey`/`getProcurementChainKey`), and the `type=assigned|activity` param handling are all gone. `GET /notifications` now merges just the three pending sources — expense approvals, procurement approvals, payment requests — with a safety-net dedupe by `link` (each pending document appears once). The badge count (`/notifications/count`) was already pending-only, so feed and badge now always agree.
- **Every pending item is enriched with "Received from" + comment** — each row from the three pending feeds carries internal `docType`/`docId` (expense id, or PI/PR/PO `request_type` + id), and `GET /notifications` attaches the **latest handover that routed the document into the user's role**:
  - `from` → the acting person (`action_by` → first/middle/last or email via `handoverActor`), `fromRole` → the role it came from, `remarks` → the sender's comment.
  - Batch lookups, one query per feed (no N+1): `buildExpenseHandoverMap` (latest `expense_handovers` per `expense_id` where `to_role_id = my role`) and `buildProcurementHandoverMap` (latest `procurement_handovers` per pending PI/PR/PO, keyed `PI:<id>`/`PR:<id>`/`PO:<id>`); `applyHandoverInfo` merges them in and strips the internal ids before returning. The count endpoint stays cheap (no enrichment).
- **`getPayments` (`expense.service.js`) payload extended** — the `processedByEmployment` include now also loads `company` (`uuid`,`name`), and each payment carries a **`processed_by_employment`** object (`{ user{first_name,middle_name,last_name,email,mobile}, company, employee_code, designation, employment_type, joining_date }` or `null`) next to the existing `processed_by` string — feeds the expense-detail "Processed by <name>" → UserDetailsModal click (see frontend 2026-09-16).

### Today's Updates (2026-09-14) — Received quantities capped at remaining
- **`updateItemsReceived` (`expense.service.js`)** — entered `received_quantity` is now an **increment** ("receiving now"), not a cumulative overwrite. It is **added** to the item's existing `received_quantity`, and **rejected (400)** when it exceeds the item's **remaining** quantity (`ordered − already received`) — error message: `Cannot receive more than the remaining quantity (<n>) for "<item>"`. e.g. 10 ordered / 5 received → next entry capped at 5. The old behaviour silently clamped each entry to 0..ordered and replaced the stored total. Joi schema unchanged (`min(0)`; the dynamic per-item cap lives in the service). `allProcurementItemsReceived`/payment gates are unaffected (they compare the cumulative `received_quantity >= quantity`).
