...
- **When the final approver closes an expense as APPROVED, it now routes to the ORIGINAL REQUESTER for payment** — `approve()` sets `current_role_id = requestedByEmployment.user.role_id` (the requester's role) instead of clearing it to `null`. The requester then sees the APPROVED expense in "My Expenses" (view only) and can either record a payment directly (if they have `expenses:pay`) or hand it over to a finance/payment role. Prior behaviour: `current_role_id` was cleared, leaving the expense with no handler.
- **`recordPayment()` now auto-returns the expense to the requester when fully settled**: when the recomputed `payment_status` is `SETTLED` (or `PAID` with `advance_amount === 0`), the expense's `status` is set to `PAID` and `current_role_id`/`current_employment_id` are set back to the requester (so the requester sees the final PAID state in "My Expenses"). Previously the handler was left wherever it was.
